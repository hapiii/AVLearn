...
#define LOG_TAG "FDKAACAudioEncoder"

FDKAACAudioEncoder::FDKAACAudioEncoder() {
}

FDKAACAudioEncoder::~FDKAACAudioEncoder() {
}

int FDKAACAudioEncoder::alloc_audio_stream(const char * codec_name) {
	AVCodec *codec;
...
int FDKAACAudioEncoder::alloc_avframe() {
	int ret = 0;
	AVSampleFormat preferedSampleFMT = AV_SAMPLE_FMT_S16;
	int preferedChannels = audioChannels;
	int preferedSampleRate = audioSampleRate;
...
int FDKAACAudioEncoder::init(int bitRate, int channels, int sampleRate, int bitsPerSample, const char* aacFilePath, const char * codec_name) {
	avCodecContext = NULL;
...
int FDKAACAudioEncoder::init(int bitRate, int channels, int bitsPerSample, const char* aacFilePath, const char * codec_name) {
	return init(bitRate, channels, 44100, bitsPerSample, aacFilePath, codec_name);
}

void FDKAACAudioEncoder::encode(byte* buffer, int size) {
	int bufferCursor = 0;
	int bufferSize = size;
...
void FDKAACAudioEncoder::encodePacket() {
...
void FDKAACAudioEncoder::destroy() {
	LOGI("start destroy!!!");
